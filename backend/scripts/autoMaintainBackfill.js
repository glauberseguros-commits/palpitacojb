...
function clampEndToTodaySP(endYmd) {
  ...
  if (!endInt || !todayInt) return { end: today, adjusted: true, reason: "invalid_end_or_today" };
...
function addDays(ymd, n) {
  // Mantém -03:00 “fixo” como você já vinha usando
...
async function main() {
  ...
  if (clamp.adjusted) {
    ...
    console.warn(
      `[WARN] Janela inclui data futura/inválida. Ajustando endDate para hoje (SP): ${end} (input=${todayRaw || "—"}, hoje=${realToday})`
    );
  }
  ...
  if (startInt && endInt && startInt > endInt) {
    console.warn(`[WARN] startDate > endDate. Ajustando startDate=endDate (${end}).`);
    start = end;
  }
  ...
  const auditFile = lastFile(`auditSlots-${lottery}-${start}_to_${end}.json`.replace(/[:]/g, ""));
