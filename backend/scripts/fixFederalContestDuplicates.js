...
async function main() {
  ...
  console.log("");
  console.log(APPLY ? "========== FEDERAL DUPLICIDADES — APPLY ==========" : "========== FEDERAL DUPLICIDADES — DRY RUN ==========");
  ...
  if (APPLY) {
    console.log("");
    console.log("Aplicando remoção dos duplicados...");

    const batchSize = 400;
    let deleted = 0;

    for (let i = 0; i < remove.length; i += batchSize) {
      const batch = db.batch();
      const chunk = remove.slice(i, i + batchSize);

      for (const item of chunk) {
        const ref = db.doc(item.refPath);

        const prizesSnap = await ref.collection("prizes").get();
        prizesSnap.forEach((pdoc) => batch.delete(pdoc.ref));
      ...
      console.log(`Removidos: ${deleted}/${remove.length}`);
    }

    console.log("");
    console.log(`CONCLUÍDO. Duplicados removidos: ${deleted}`);
  }
