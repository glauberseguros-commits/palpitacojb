...
function normalizeCloseHourForPtRio(value) {
...
async function readFirestoreByDate(date) {
  ...
  for (const doc of snap.docs) {
    ...
    const slot = normalizeCloseHourForPtRio(d.close_hour || d.close || d.hour || "");
    if (!slot) continue;
...
async function auditDate(date) {
  ...
  for (const draw of sourceDraws) {
    const slot = normalizeCloseHourForPtRio(draw?.close_hour || "");
    if (!slot) continue;
