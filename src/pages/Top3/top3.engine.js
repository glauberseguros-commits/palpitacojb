  ...
  const ranked = Array.from({ length: safeInt(TOP3_GROUPS_K, 25) }, (_, idx) => {
    ...
    for (const keyLayer of Object.keys(layers)) {
      ...
      layerDetails.sceneRetention =
        1 - Number(effectiveSceneWeight || 0);
    ...
    details.scene = {
      ...
      contribution:
        pScene * Number(effectiveSceneWeight || 0),
      scoreContribution:
        pScene * Number(effectiveSceneWeight || 0) * 1000,
    };
 ...
 *
 * PT_SP is isolated from all foreign specialist engines.
 * The certified PT_SP Production V1 uses the generic V3
 * base directly and preserves the current-main route for
 * every non-PT_SP lottery.
 */
