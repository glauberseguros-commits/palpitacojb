 ...
 * Uso:
 *  DRY RUN (não grava):
 *    node scripts/maintenance/backfillYmdDraws.js --dry
 *
 *  Gravar de fato:
 *    node scripts/maintenance/backfillYmdDraws.js
 *
 *  Opcional:
 ...
 * Evita “virar o dia” quando roda em UTC.
 */
...
function normalizeToYMD(input) {
  ...
  if (brDash) {
    const ymd = `${brDash[3]}-${brDash[2]}-${brDash[1]}`;
    return isValidYMD(ymd) ? ymd : null;
  }
...
async function main() {
  ...
  while (true) {
    ...
    if (!writes.length) {
      console.log(
        `[PAGE] docs=${snap.size} | processed=${processed} | writes=0 | last=${lastDoc?.id || "n/a"}`
      );
      continue;
    }

    console.log(
      `[PAGE] docs=${snap.size} | processed=${processed} | writes=${writes.length} | last=${lastDoc?.id || "n/a"}`
    );
