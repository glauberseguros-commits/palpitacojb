...
const SORTS = Object.freeze([
  ...
  Object.freeze({ value: "number_asc", label: "N├║mero crescente" }),
  Object.freeze({ value: "number_desc", label: "N├║mero decrescente" }),
  Object.freeze({ value: "latest_desc", label: "├Ültima apari├º├úo" }),
]);
...
export default function Statistics() {
    ...
    (async () => {
      try {
        ...
        if (!normalized.minYmd || !normalized.maxYmd) {
          throw new Error(
            `Per├¡odo hist├│rico n├úo encontrado para ${lotteryKey}.`
          );
        }
        ...
        setError(
          safeStr(caught?.message || caught) ||
            "Falha ao carregar o per├¡odo hist├│rico."
        );
      }
    })();
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      if (!boundsReady) {
        throw new Error(
          "O per├¡odo hist├│rico ainda n├úo foi carregado."
        );
      }
      ...
      if (!isYmd(from) || !isYmd(to)) {
        throw new Error("Intervalo de datas inv├ílido.");
      }

      if (from > to) {
        throw new Error(
          "A data inicial n├úo pode ser posterior ├á data final."
        );
      }
      ...
      if (!chunks.length) {
        throw new Error(
          "N├úo foi poss├¡vel dividir o per├¡odo para an├ílise."
        );
      }
      ...
      setError(
        safeStr(caught?.message || caught) ||
          "Falha ao calcular as frequ├¬ncias."
      );
    } finally {
      if (!isCurrent()) return;
  ...
  const copyRanking = useCallback(async () => {
    ...
    try {
      ...
      alert(
        isUnseenView
          ? "Milhares in├®ditas copiadas."
          : "Ranking copiado."
      );
    } catch {
      alert(
        "N├úo foi poss├¡vel copiar automaticamente."
      );
    }
  }, [
    isUnseenView,
    ...
    <div className="ppStatistics">
      ...
      <section className="ppStatsPanel">
        <header className="ppStatsHeader">
          <div>
            <div className="ppStatsTitle">Frequ├¬ncias</div>
            <div className="ppStatsSubtitle">
              Ranking hist├│rico conforme a loteria, per├¡odo, hor├írio,
              animal e posi├º├úo selecionados.
            </div>
          </div>
        ...
        {mode === "milhar" ? (
          <div
            ...
            role="tablist"
            aria-label="Visualiza├º├úo das milhares"
          >
            <button
              type="button"
              className={[
                      ...
                      <tr>
                        <td colSpan={4}>
                          <div className="ppStatsEmpty">
                            {selectedPosition === 7
                              ? "O 7┬║ pr├¬mio possui apenas centena, n├úo milhar."
                              : rows.length
                              ? "Todas as milhares do universo selecionado j├í apareceram."
                              : "Selecione os filtros e toque em ÔÇ£Gerar rankingÔÇØ."}
                          </div>
                        </td>
                      </tr>
                    ) : null}
