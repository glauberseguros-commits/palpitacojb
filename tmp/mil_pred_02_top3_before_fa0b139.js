...
/**
 * Calend├írio Federal com preserva├º├úo hist├│rica.
 *
 * At├® 18/07/2026:
 * - quarta-feira ├ás 20h;
 * - s├íbado ├ás 20h.
 ...
 * - quarta-feira ├ás 20h;
 * - domingo ├ás 11h;
 * - s├íbado deixa de ter sorteio Federal.
 */
 ...
 * Nenhum resultado hist├│rico ├® removido.
 */
...
/* =========================
   Pr├│ximo DRAW REAL (n├úo perde amostra)
========================= */
  ...
  return [
    {
      ...
      label: "dia do m├¬s + dia da semana + hora anterior + grupo anterior -> alvo",
...
export function computeConditionalNextTop3({
  ...
  const reasonsBase = [
    `Camada usada: ${chosen?.label || "ÔÇö"}`,
    `Amostra hist├│rica condicional: ${samples}`,
    ...
    `Estado atual: prev=${String(prevGrupo).padStart(2, "0")} @ ${lastH} ÔåÆ alvo ${targetH}`,
  ...
  const top = ranked.map((x, idx) => {
    ...
    return {
      ...
      freqZeroWhy:
        x.aparicoes <= 0
          ? `Sem ocorr├¬ncia nesta camada (${chosen?.label || "ÔÇö"}).`
          : "",
      reasons: [
        ...reasonsBase,
        `Grupo G${g2}: apari├º├Áes=${x.aparicoes} em ${samples} amostras do pr├│ximo sorteio`,
        `Grupo G${g2}: primeiros lugares=${x.primeiros}`,
        `Probabilidade final estimada no TOP3: ${pct}%`,
        `Composi├º├úo da probabilidade: condicional=${pctCond}% | estrutural=${pctBase}%`,
        `Presen├ºa indireta recente (2┬║-4┬║): ${x.recentIndirect}`,
        `Persist├¬ncia curta recente: ${x.recentPrizePresence}`,
        `B├┤nus de atraso ajustado: ${(Number(x.smartLateBoost || 0) * 100).toFixed(2)}%`,
      ],
      meta: {
        trigger: {
          ymd: lastY,
...
/* =========================
   V2 ÔÇö helpers focados em BICHO
========================= */
...
export function computeConditionalNextTop3V2({
  ...
  const reasonsBase = [
    ...
    `Pesos: mode=${mode} | transi├º├úo=${(adjustedWeights.transition * 100).toFixed(0)}% | par=${(adjustedWeights.pair * 100).toFixed(0)}% | mem├│ria=${(adjustedWeights.memory * 100).toFixed(0)}% | rec├¬ncia=${(adjustedWeights.recent * 100).toFixed(0)}% | estrutural=${(adjustedWeights.structural * 100).toFixed(0)}% | atraso=${(adjustedWeights.late * 100).toFixed(0)}%`,
    `Estado atual: prev=${String(prevGrupo).padStart(2, "0")} @ ${lastH} ÔåÆ alvo ${targetH}`,
    `Estado curto compar├ível: [${currentState.map((g) => String(g).padStart(2, "0")).join(", ")}]`,
    `Camada condicional: ${chosen?.label || "ÔÇö"} | amostras=${condSamples}`,
    `Amostra estrutural 1┬║=${Number(structuralFirst?.totalSamples || 0)} | PRIZE_PRESENCE=${Number(structuralPrizePresence?.totalSamples || 0)} | duplica├º├úo=${Number(duplication?.totalSamples || 0)}`,
    `Amostra mem├│ria: matches=${Number(memoryOut?.matchedSamples || 0)} | peso=${Number(memoryOut?.totalWeight || 0).toFixed(2)}`,
    `Par do dia: ${Array.isArray(prevPair) && prevPair.length === 2 ? prevPair.map((n) => String(n).padStart(2, "0")).join("ÔåÆ") : "--"} | matches=${Number(pairOut?.samples || 0)} | matches DOM=${Number(pairOut?.exactDomSamples || 0)} | confian├ºa=${(pairConfidence * 100).toFixed(0)}%`,
    `Fluxo do dia: draws=${Number(dayContext?.total || 0)} | confian├ºa=${(dayFlowConfidence * 100).toFixed(0)}%`,
    `Data alvo: ${targetY} | DOW=${targetDow} | dia=${String(targetDayOfMonth).padStart(2, "0")}`,
  ];
  ...
  const top = ranked.map((x, idx) => {
    ...
    return {
      ...
      freqZeroWhy:
        x.condFirstCount <= 0
          ? `Sem 1┬║ lugar nesta camada (${chosen?.label || "ÔÇö"}).`
          : "",
      reasons: [
        ...reasonsBase,
        `Grupo G${g2}: transi├º├úo para 1┬║=${(x.probTransition * 100).toFixed(2)}%`,
        `Grupo G${g2}: estrutural de 1┬║=${(x.probStructuralFirst * 100).toFixed(2)}%`,
        `Grupo G${g2}: estrutural de TOP3=${(x.probStructuralPrizePresence * 100).toFixed(2)}%`,
        `Grupo G${g2}: duplica├º├úo hist├│rica=${(x.probDuplication * 100).toFixed(2)}%`,
        `Grupo G${g2}: sequ├¬ncia ordem 2=${(x.probSeq2 * 100).toFixed(2)}%`,
        `Grupo G${g2}: par do dia=${(x.probPair * 100).toFixed(2)}% | confian├ºa=${(x.pairConfidence * 100).toFixed(2)}%`,
        `Grupo G${g2}: boost de repeti├º├úo=${(x.repeatBoost * 100).toFixed(2)}%`,
        `Grupo G${g2}: mem├│ria curta=${(x.probMemory * 100).toFixed(2)}%`,
        `Grupo G${g2}: rec├¬ncia composta=${(x.recentComposite * 100).toFixed(2)}%`,
        `Grupo G${g2}: atraso normalizado=${(x.lateNorm * 100).toFixed(2)}%`,
        `Grupo G${g2}: 1┬║s na camada=${x.condFirstCount}`,
        `Grupo G${g2}: 1┬║s estruturais=${x.structuralFirstCount}`,
        `Grupo G${g2}: TOP3 estruturais=${x.structuralPrizePresenceCount}`,
        `Grupo G${g2}: draws com duplica├º├úo=${x.duplicationCount}`,
        `Grupo G${g2}: peso de mem├│ria=${Number(x.memoryWeight || 0).toFixed(2)}`,
        `Grupo G${g2}: frequ├¬ncia no dia=${x.dayFreq}`,
        `Grupo G${g2}: 1┬║s no dia=${x.dayFirstFreq}`,
        `Grupo G${g2}: dominante no dia=${x.isDominantToday ? "sim" : "n├úo"}`,
        `Grupo G${g2}: score do fluxo do dia=${Number(x.dayFlowScore || 0).toFixed(2)} | confian├ºa=${(Number(x.dayFlowConfidence || 0) * 100).toFixed(0)}%`,
        `Grupo G${g2}: sequ├¬ncia ${Array.isArray(prevPair) && prevPair.length === 2 ? prevPair.map((n) => String(n).padStart(2, "0")).join("ÔåÆ") : "--"} | amostras=${x.pairSamples} | amostras ponderadas=${Number(x.pairWeightedSamples || 0).toFixed(2)} | DOM exato=${x.pairExactDomSamples} | ocorr├¬ncias=${Number(x.pairCount || 0).toFixed(2)} | prob=${(Number(x.pairProb || 0) * 100).toFixed(2)}% | score=${Number(x.pairSequenceScore || 0).toFixed(2)}`,
...
export function computeStatisticalTop3V3({
  ...
  const layers = {
    hour: { label: `frequ├¬ncia no hor├írio ${targetH}`, samples: 0, first: emptyGroupMap(), prizePresence: emptyGroupMap(), weight: 0.33 },
    dowHour: { label: `frequ├¬ncia no dia da semana + hor├írio`, samples: 0, first: emptyGroupMap(), prizePresence: emptyGroupMap(), weight: 0.27 },
    dayMonth: { label: `frequ├¬ncia no dia ${String(targetDayOfMonth).padStart(2, "0")}`, samples: 0, first: emptyGroupMap(), prizePresence: emptyGroupMap(), weight: 0.04 },
    transition: { label: `transi├º├úo G${String(prevGrupo).padStart(2, "0")} @ ${lastH} ÔåÆ ${targetH}`, samples: 0, first: emptyGroupMap(), prizePresence: emptyGroupMap(), weight: 0.26 },
    recent: { label: `rec├¬ncia compar├ível`, samples: 0, first: emptyGroupMap(), prizePresence: emptyGroupMap(), weight: 0.10 },
  };
  ...
  const ranked = Array.from({ length: safeInt(TOP3_GROUPS_K, 25) }, (_, idx) => {
    ...
    details.scene = {
      label: "analogia hist├│rica de cena",
  ...
  const top = rankedScored.map((x, idx) => {
    ...
    return {
      ...
      reasons: [
        ...
        ...strongest.map((d) =>
          `${d.label}: ${d.firstCount}x em 1┬║ | ${d.top3Count}x no TOP3 | amostras=${d.samples} | peso=${(d.weight * 100).toFixed(0)}%`
        ),
      ],
      meta: {
        trigger: {
...
/* =========================
   16/20 milhares (por grupo) ÔÇö POR TERMINA├ç├âO (CORRETO)
========================= */
