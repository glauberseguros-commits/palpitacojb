...
helper_anchor = """function getFirstGrupoFromDraw(draw) {
...
helper_replacement = """function getFirstGrupoFromDraw(draw) {
  const ps = Array.isArray(draw?.prizes) ? draw.prizes : [];
  const p1 = ps.find((p) => guessPrizePos(p) === 1) || null;
  if (p1) {
    const g = guessPrizeGrupo(p1);
...
function getPrizeGroupsByPosition(draw, maxPosition = 3) {
  ...
  return Array.from({ length: limit }, (_, index) => {
    ...
    return Number.isFinite(grupo) && grupo >= 1 && grupo <= TOP3_GROUPS_K
      ? grupo
      : null;
  });
}
...
if helper_anchor not in text:
    raise SystemExit(
        "Âncora do helper não encontrada. Nenhuma alteração foi aplicada."
    )

text = text.replace(helper_anchor, helper_replacement, 1)

audit_pattern = re.compile(
    r"""export function auditTop3Timeline\(\{\s*
  timeline,\s*
  lotteryKey = "",\s*
\}\) \{.*?
\n\}\n\n
...
audit_replacement = r'''export function auditTop3Timeline({
  timeline,
  lotteryKey = "",
}) {
  ...
  function normalizeResultTop3(slot) {
    const explicit = Array.isArray(slot?.resultTop3Groups)
  ...
  function evaluateSlot(slot) {
    ...
    const top3Hit =
      Number.isFinite(resultGrupo) &&
      picks.some((grupo) => Number(grupo) === resultGrupo);

    const prizePositionHits = resultTop3.map(
      (grupo) =>
        Number.isFinite(Number(grupo)) &&
        picks.some((pick) => Number(pick) === Number(grupo))
    );
    ...
    const matchedPrizePositions = prizePositionHits.filter(Boolean).length;
    const matchedPredictions = predictionHits.filter(Boolean).length;
  ...
  function summarize(items) {
    ...
    for (const item of items) {
      ...
      if (metrics.top3PrizeHit) summary.top3PrizeHits += 1;

      summary.matchedPrizePositions += Number(
        metrics.matchedPrizePositions || 0
      );

      summary.matchedPredictions += Number(
        metrics.matchedPredictions || 0
      );
    }
    ...
    return {
      ...
      top3PrizeRate: pct(summary.top3PrizeHits, summary.total),
      averageMatchedPrizePositions: summary.total
        ? Number(
            (
              summary.matchedPrizePositions / summary.total
            ).toFixed(4)
          )
        : 0,
      averageMatchedPredictions: summary.total
        ? Number(
            (
              summary.matchedPredictions / summary.total
            ).toFixed(4)
          )
        : 0,
    };
  ...
  return {
    ...
    rows: evaluated.map(({ slot, metrics }) => ({
      ...
      matchedPrizePositions: metrics.matchedPrizePositions,
      matchedPredictions: metrics.matchedPredictions,

      prizePositionHits: metrics.prizePositionHits,
      predictionHits: metrics.predictionHits,
...
text, audit_replacements = audit_pattern.subn(
    audit_replacement,
    text,
    count=1,
)

if audit_replacements != 1:
    raise SystemExit(
        "Bloco auditTop3Timeline não localizado de forma única. "
        "Nenhuma alteração segura foi concluída."
    )

result_anchor = """    const resultGrupo = currentDraw ? pickPrize1GrupoFromDraw(currentDraw) : null;
...
result_replacement = """    const resultTop3Groups = currentDraw
    ...
    const hit =
      Number.isFinite(Number(normalizedResultGrupo)) && top3.length
        ? top3.some(
            (t) =>
              Number(t?.grupo) === Number(normalizedResultGrupo)
          )
        : null;
    ...
    timeline.push({
      ...
      resultTop3Groups,
      hit,
      status: Number.isFinite(Number(normalizedResultGrupo))
        ? "validated"
        : "pending",
      historyStats,
    });
...
if result_anchor not in text:
    raise SystemExit(
        "Âncora da gravação do resultado não encontrada. "
...
print("Auditoria ampliada para posições reais 1, 2 e 3.")
